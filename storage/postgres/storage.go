...
// NEW
func (s *storageRepo) GetByProductID(ctx context.Context, pKey models.PrimaryKey) (models.Storage, error) {
	storage := models.Storage{}

	query := `SELECT id, product_id, branch_id, count
	from storages where product_id = $1`
	...
	if err != nil {
		fmt.Println("Error while gettin storage by product id!", err.Error())
		return models.Storage{}, err
	}
...
func (s *storageRepo) UpdateCount(ctx context.Context, storageID string, count int) error {
	query := `UPDATE storages set count = count - $1 where id = $2`

	_, err := s.DB.Exec(ctx, query, count, storageID)
	if err != nil {
		fmt.Println("Error while updating product count of storage!", err.Error())
		return err
	}
	return nil
}
